...
ARCHIVE_EXTENSIONS = [".7z", ".zip", ".rar", ".gz", ".tar", ".tgz", ".xz", ".bz2"]
...
def parse_sample_dir(dir, notRecursive=False, generateInfo=False, onlyRelevantExtensions=False):
    ...
    for filePath in get_files(dir, notRecursive):
        try:
            ...
            # MalwareBazaar samples are often downloaded as password-protected .7z/.zip archives.
            # yarGen must analyze the extracted payload, not the archive container.
            if extension in ARCHIVE_EXTENSIONS:
                if args.debug:
                    print("[-] ARCHIVE %s - Skipping archive file %s (extract it first, usually password: infected)" % (extension, filePath))
                continue
...
def extract_strings(fileData):
    ...
    try:
        ...
        strings_limited = re.findall(b"[\x1f-\x7e]{6,%d}" % int(args.s), fileData)
...
def generate_rules(file_strings, file_opcodes, super_rules, file_info, inverse_stats):
    ...
    if args.o:
        try:
            ...
            fh = open(args.o, 'w', encoding='utf-8')
        except Exception as e:
            traceback.print_exc()
    ...
    if not args.inverse:
        ...
        for filePath in file_strings:
            ...
            try:
                ...
                condition_pe_part1 = []
                condition_pe_part2 = []
                if not args.noextras and file_info[filePath]["magic"] == "4d5a":
...
def is_ascii_char(b, padding_allowed=False):
    if padding_allowed:
        if (b[0] < 127 and b[0] > 31) or b[0] == 0:
            return 1
    else:
        if b[0] < 127 and b[0] > 31:
            return 1
    return 0
...
def is_ascii_string(string, padding_allowed=False):
    for b in [i.to_bytes(1, sys.byteorder) for i in string]:
        if padding_allowed:
            if not ((b[0] < 127 and b[0] > 31) or b[0] == 0):
                return 0
        else:
            if not (b[0] < 127 and b[0] > 31):
                return 0
    return 1
...
def removeNonAsciiDrop(string):
    ...
    try:
        ...
        nonascii = b"".join(i for i in byte_list if i[0]<127 and i[0]>31)
    except Exception as e:
        traceback.print_exc()
        pass
    return nonascii
...
if __name__ == '__main__':
    ...
    # Python 3 compatibility: argparse returns strings for these legacy yarGen parameters.
    # Convert them once so regex formatting, size checks, and comparisons do not crash.
    for _name in ("y", "z", "x", "w", "s", "rc", "fm", "fs", "n"):
        try:
            setattr(args, _name, int(getattr(args, _name)))
        except Exception:
            pass
