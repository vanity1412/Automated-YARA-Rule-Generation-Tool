...
class GenerateScreen(ttk.Frame):
    ...
    def build(self):
        ...
        ttk.Label(self, text=self.app.t("generate.title"), style="Title.TLabel").grid(row=0, column=0, sticky="w")
        top = ttk.Frame(self, style="Card.TFrame", padding=12)
        top.grid(row=1, column=0, sticky="ew", pady=8)
        ...
        ttk.Combobox(top, values=["Full quality DB", "Fast strings DB", "Fast no-opcodes DB"], state="readonly", textvariable=self.app.state.var_db_mode, width=20).grid(row=0, column=3, padx=4)
        ...
        adv.grid(row=0, column=1, sticky="nsew", padx=(6, 0))
        ...
        flags = ttk.LabelFrame(self, text="Options", padding=12)
        flags.grid(row=3, column=0, sticky="nsew", pady=8)
        options = [
        ...
        for idx, (txt, var) in enumerate(options):
            cb = ttk.Checkbutton(flags, text=txt, variable=var, command=self.update_command_preview)
            cb.grid(row=idx // 4, column=idx % 4, sticky="w", padx=10, pady=3)
            if txt in {"--debug", "--trace", "--noscorefilter"}:
                self.advanced_widgets.append(cb)

        cmd_box = ttk.LabelFrame(self, text="Command Preview", padding=8)
        cmd_box.grid(row=4, column=0, sticky="ew")
        cmd_box.columnconfigure(0, weight=1)
        ttk.Entry(cmd_box, textvariable=s.var_command_preview).grid(row=0, column=0, sticky="ew", padx=(0, 6))
